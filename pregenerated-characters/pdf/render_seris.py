"""Two A4 pages for Seris Vale, following the approved Tamsin layout."""
...
OUT = ROOT / "output/pdf/seris-vale-a4.pdf"
TMP = ROOT / "tmp/pdfs/seris"
PORTRAIT = ROOT / "pregenerated-characters/pdf/assets/seris-vale-portrait.png"
TMP.mkdir(parents=True, exist_ok=True)
OUT.parent.mkdir(parents=True, exist_ok=True)
audited = next(a for a in json.loads((ROOT / "pregenerated-characters/audit.json").read_text()) if a["name"] == "Seris Vale")
...
def footer(page):
    ...
    c.setFont("Bold", 7.1)
    c.drawRightString(W-M, 13, f"SERIS VALE  /  {page} OF 2")
...
text("THE LAST WAGON COMPANY  /  THE CAREFUL TRAIL", M, H-35, 9.2, "Bold", ROSE)
text("Seris Vale", M, H-68, 30, "Display")
text("Elf  /  25  /  Woodland guide, tracker & archer", M, H-85, 10, "Body", MUTED)
...
y = hero_top
for paragraph in [
    "Lean and angular, with copper-brown skin, dark eyes, and ash-blond hair cut below her ears. Her green-grey travelling clothes have practical brown repairs. She becomes very still when listening.",
    "**Personality:** Wry, attentive, and unexpectedly talkative over a meal. She explains tracks and weather plainly. Her weakness is distrusting a convenient shortcut until she has checked it herself.",
    "**Background:** Seris grew up guiding the wooded approaches to Whitewater. She led the flooded caravan towards firmer ground and joined the charter instead of taking another solitary season.",
    "**Goal:** Make the routes between isolated households safer for everyone who travels them.",
    "**Bond:** Ilen returned along a dangerous path to correct a message. She trusts him to report when a route has changed.",
]:
    y = para(paragraph, bx, y, bw, size=9.8, leading=12.3, after=5)
y = para('"Something passed here. Let us work out how recently."', bx, y, bw, size=10, leading=12.5, color=TEAL, after=0)
y = para("**Mother tongue:** River Crown · **Familiar craft:** Bowyer", bx, y-9, bw, size=9.3, leading=11.8, color=MUTED, after=0)
...
y = grid(["ATTACK", "SKILL", "DAMAGE", "SIZE / RANGE"], [
    ["Longspear", "44%", "1D8+1", "Medium / 2 m"],
    ["Longbow", "60%", "1D10", "- / 150 m"],
    ["Dagger", "44%", "1D4+1", "Light / 2 m"],
    ["Thrown dagger", "60%", "1D4+1", "Light / 13 m"],
    ["Unarmed", "34%", "1D3", "Light / 2 m"],
], [82,33,48,COL-163], x, y, 8.9, 10.8, 2.0)
y = para("**Ready:** Longbow held, no arrow nocked; longspear carried.", x, y, COL, size=9.6, leading=12, after=5)
y = heading("Talents & ancestry", x, y, COL)
for talent in ["Low-Light Sight", "Terrain Expertise (Forest)", "Tracker", "Wayfinder"]:
...
y = grid(["CARRIED", "QUANTITY / SUPPLY"], [
    ...
    ["Longspear, longbow, dagger", "1 each"],
    ["Backpack", "1"],
    ["Rope", "10 m"],
    ...
    ["Bedroll", "1"],
    ["Climbing kit", "1"],
    ["Healing kits", "2 kits; 10 uses total"],
    ["Fishing kit", "1"],
    ["Lantern and oil", "1 each"],
    ["Quiver and arrows", "1 quiver; 30 arrows"],
    ["Grappling hook", "1"],
], [COL*.57, COL*.43], x, y, 9.2, 11.2, 1.7)
y = para("**Money:** 28 SP\n**Listed load:** 16 ENC · **Capacity:** 22 ENC", x, y, COL, size=9.6, leading=12.2, after=5)
...
text("SERIS VALE", M, H-35, 9.2, "Bold", ROSE)
...
groups = [
    ...
    [("Practical, continued", practical[7:]), ("Craft", [s for s in skills if s["name"]=="Craft (Bowyer)"]), ("Knowledge", [s for s in skills if s["name"] in {"Lore (Woodcraft)", "Natural Lore"}]), ("Culture", [s for s in skills if s["name"] in {"Culture (Whitewater)"}]), ("Language", [s for s in skills if s["name"]=="Language (River Crown)"])],
]
ends = []
...
notes = [
    ("Low-Light Sight", "For Seris, **Partial Darkness counts as Illuminated**, and **Darkness as Partial Darkness**. Pitch Black and obscurity are unchanged."),
    ("Terrain Expertise (Forest)", "On **Natural Lore tests in or about forests**, remove up to **1 remaining Penalty die** after ordinary Bonus/Penalty cancellation and before the final dice cap. This never creates Bonus dice and applies to no other terrain. Terrain Expertise and Tracker can both apply to the same test."),
    ("Tracker", "On **Natural Lore tests to find or follow a trail**, remove up to **1 remaining Penalty die** after ordinary Bonus/Penalty cancellation and before the final dice cap. This never creates Bonus dice and applies to no other use of Natural Lore."),
    ("Wayfinder", "While Seris can **see the sky or read the ground**, downgrade a navigation Fumble to an ordinary Failure. This prevents the group from becoming lost through that test. Other travel hazards remain: weather, pace, and finding food, water, or shelter use their ordinary rules."),
]
card_h = 146
...
expected_skills = {
    ...
    or skill["name"] in {"Craft (Bowyer)", "Lore (Woodcraft)", "Natural Lore", "Culture (Whitewater)", "Language (River Crown)"}
}
...
writer.add_metadata({"/Title":"Seris Vale - The Careful Trail", "/Author":"Fantasy Crux 2.0 - Last Wagon Company", "/Subject":"Two-page A4 character sheet with portrait, skills, talents and equipment", "/Keywords":"Fantasy Crux, Seris Vale, Last Wagon Company, character sheet, A4"})
