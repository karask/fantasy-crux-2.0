"""Two A4 pages for Mara Holt, following the approved Tamsin layout."""
...
OUT = ROOT / "output/pdf/mara-holt-a4.pdf"
TMP = ROOT / "tmp/pdfs/mara"
PORTRAIT = ROOT / "pregenerated-characters/pdf/assets/mara-holt-portrait.png"
TMP.mkdir(parents=True, exist_ok=True)
OUT.parent.mkdir(parents=True, exist_ok=True)
audited = next(a for a in json.loads((ROOT / "pregenerated-characters/audit.json").read_text()) if a["name"] == "Mara Holt")
...
def footer(page):
    ...
    c.setFont("Bold", 7.1)
    c.drawRightString(W-M, 13, f"MARA HOLT  /  {page} OF 2")
...
text("THE LAST WAGON COMPANY  /  THE STEADY HAND", M, H-35, 9.2, "Bold", ROSE)
text("Mara Holt", M, H-68, 30, "Display")
text("Human  /  27  /  Road-watch escort & shield protector", M, H-85, 10, "Body", MUTED)
...
y = hero_top
for paragraph in [
    "Broad-shouldered and square-faced, with close-cropped brown hair, a crooked healed nose, and a pale rope scar across her left palm. Her leather armour is carefully restitched at the right shoulder.",
    "**Personality:** Patient, practical, and quietly funny. She mends other people's straps and remembers who dislikes cold tea. Under stress, she gives jobs before asking what others have noticed.",
    "**Background:** Mara left the road watch when ordered to save a merchant's cargo before stranded passengers. During the flood, she braced the wagon and returned for its last injured guard.",
    "**Goal:** Make the company's charter a welcome sight to frightened travellers, regardless of who can pay.",
    "**Bond:** Pebb talked a hostile tollkeeper into opening shelter during the flood. Mara trusts their judgement and gives negotiations time.",
]:
    y = para(paragraph, bx, y, bw, size=9.8, leading=12.3, after=5)
y = para('"Tell me when you are ready. I will hold."', bx, y, bw, size=10, leading=12.5, color=TEAL, after=0)
...
y = grid(["ATTACK", "SKILL", "DAMAGE", "SIZE / RANGE"], [
    ["Arming sword", "58%", "1D8+1D4+1", "Medium / 2 m"],
    ["Shortbow", "39%", "1D8", "- / 75 m"],
    ["Dagger", "58%", "2D4+1", "Light / 2 m"],
    ["Thrown dagger", "39%", "2D4+1", "Light / 15 m"],
    ["Large shield", "58%", "1D6+1D4", "Huge / 2 m"],
    ["Unarmed", "33%", "1D3+1D4", "Light / 2 m"],
], [72,33,68,COL-173], x, y, 8.9, 10.8, 2.0)
y = para("**Ready:** Arming sword and large shield; shortbow slung.", x, y, COL, size=9.6, leading=12, after=5)
y = heading("Talents", x, y, COL)
for talent in ["Protector", "Shield Cover", "Favoured Weapon (Arming Sword)", "Athletics Expertise (Climbing)"]:
...
y = grid(["CARRIED", "QUANTITY / SUPPLY"], [
    ...
    ["Arming sword, shortbow, dagger", "1 each"],
    ["Large shield", "1"],
    ["Backpack", "1"],
    ["Rope", "10 m"],
    ...
    ["Quiver and arrows", "1 quiver; 20 arrows"],
], [COL*.57, COL*.43], x, y, 9.2, 11.2, 1.7)
y = para("**Money:** 74 SP\n**Listed load:** 18 ENC · **Capacity:** 29 ENC", x, y, COL, size=9.6, leading=12.2, after=5)
...
text("MARA HOLT", M, H-35, 9.2, "Bold", ROSE)
...
groups = [
    ...
    [("Practical, continued", practical[7:]), ("Craft", [s for s in skills if s["name"]=="Craft (Tailor)"]), ("Knowledge", [s for s in skills if s["name"] in {"Lore (Military Tactics)", "Natural Lore"}]), ("Culture", [s for s in skills if s["name"]=="Culture (Whitewater)"]), ("Language", [s for s in skills if s["name"]=="Language (River Crown)"])],
]
ends = []
...
notes = [
    ("Protector", "When an adjacent ally is hit by an attack Mara is aware of, spend her **base Reaction** to Parry with her ready shield at **58%**. Success reduces damage normally; failure leaves the ally hit. Ranged attacks use Active Guard eligibility and modifiers. The ally keeps their Reaction, but cannot also React to this attack. Mara's extra off-hand Reaction cannot be used."),
    ("Shield Cover", "While Mara is aware and her large shield is ready, ranged attacks and individually targeted Projected Shaping against her suffer **-1P**. This costs no Reaction and applies before Active Guard. Use only the best Shield Cover or terrain penalty. Dodge and Active Guard remain available. This gives no protection against areas or Direct Harm."),
    ("Favoured Weapon (Arming Sword)", "Damaging attacks with Mara's arming sword gain **+1 damage** before Parry and armour, already included on page 1. This includes a primitive arming sword, but not use as an improvised club. No bonus applies when normal damage is replaced, including Disarm, Trip, Shield Rush, Subdue, or an unopposed Critical. A Critical opposed by an ordinary Reaction retains the bonus."),
    ("Athletics Expertise (Climbing)", "On **Athletics tests for climbing**, remove up to **1 remaining Penalty die** after ordinary Bonus/Penalty cancellation and before the final dice cap. This never creates Bonus dice and does not apply to other Athletics uses."),
]
card_h = 164
for i, (name, description) in enumerate(notes):
    ...
    assert z >= top-card_h+7, f"Character card overflow: {name}"
...
expected_skills = {
    ...
    or skill["name"] in {"Craft (Tailor)", "Lore (Military Tactics)", "Natural Lore", "Culture (Whitewater)", "Language (River Crown)"}
}
...
writer.add_metadata({"/Title":"Mara Holt - The Steady Hand", "/Author":"Fantasy Crux 2.0 - Last Wagon Company", "/Subject":"Two-page A4 character sheet with portrait, skills, talents and equipment", "/Keywords":"Fantasy Crux, Mara Holt, Last Wagon Company, character sheet, A4"})
